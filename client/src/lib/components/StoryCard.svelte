<script lang="ts">
	...
	import {
		...
		moveCardOnBoardFinal,
		moveCardOnBoardState,
		removeCardFromBoard,
		writeInCard
	...
	const handleBlur = async () => {
		if (inputEl.value.trim() === '') {
			...
			await writeInCard(card, inputEl.value);
		}
		await changeCardHeight(card, inputEl.scrollHeight);
	};
	...
	use:draggable={{
		...
		listeners: {
			...
			move(event) {
				...
				moveCardOnBoardState(card.id, { x: startX + x, y: startY + y });
			},
			end(event) {
				const el = event.target as HTMLElement;
				const startX = parseFloat(el.dataset.startX!) || 0;
				...
				moveCardOnBoardFinal(card, { x: startX + x, y: startY + y });
